...
class IBKRAdapter:
    ...
    def __init__(self, host='127.0.0.1', port=7497, client_id=1, 
             alpaca_key_id=None, alpaca_secret_key=None, logger=None):
        """
        Initialize IBKR adapter with Alpaca credentials for market data
        
        Args:
            host: IB Gateway host (default: '127.0.0.1')
            port: IB Gateway port (default: 7497 for paper, 7496 for live)
            client_id: Unique client ID (default: 1)
            alpaca_key_id: Alpaca API key for market data prices
            alpaca_secret_key: Alpaca secret key for market data prices
    ...
    def list_positions(self):
        """
        Get current positions with P&L calculated using Alpaca market prices
        """
        ...
        if self.alpaca_key_id and self.alpaca_secret_key:
            try:
                ...
                symbols = [p.contract.symbol for p in ibkr_positions if p.contract.symbol]
                
                if symbols:
                    for symbol in symbols:
                        try:
                            trade = alpaca.get_latest_trade(symbol)
                            market_prices[symbol] = trade.price
                        except Exception as e:
                            self.logger.debug(f"Could not get price for {symbol}: {e}")
                    
                    self.logger.info(f"✅ Got market prices from Alpaca for {len(market_prices)}/{len(symbols)} symbols")
            
            except Exception as e:
                self.logger.warning(f"Alpaca market data failed: {e}, using avgCost")
        else:
            self.logger.warning("No Alpaca credentials provided, using avgCost for P&L")
    
    # ... reste du code avec class Position ...
        
        class Position:
            def __init__(self, ibkr_pos, market_prices):
                self.symbol = ibkr_pos.contract.symbol
                self.qty = str(abs(int(ibkr_pos.position)))
                self.side = 'long' if ibkr_pos.position > 0 else 'short'
                
                # Get market price (Alpaca or fallback to avgCost)
                if self.symbol in market_prices:
                    market_price = market_prices[self.symbol]
                else:
                    market_price = abs(ibkr_pos.avgCost)
                
                # Average entry price
                self.avg_entry_price = str(abs(ibkr_pos.avgCost))
                
                # Current price
                self.current_price = str(market_price)
                
                # Market value
                self.market_value = str(ibkr_pos.position * market_price)

                # ✅ CALCULATE P&L using Alpaca prices
                pnl = ibkr_pos.position * (market_price - abs(ibkr_pos.avgCost))
                self.unrealized_pl = str(pnl)

                # Cost basis
                self.cost_basis = str(abs(ibkr_pos.position * ibkr_pos.avgCost))

                # P&L percentage
                cost_basis = abs(ibkr_pos.position * ibkr_pos.avgCost)
                if cost_basis > 0:
                    self.unrealized_plpc = str(pnl / cost_basis)
                else:
        ...
        return [Position(p, market_prices) for p in ibkr_positions]
